...
이벤트 화면, 시간순 폴트 목록과 선택 항목별 상세 해설을 제공하는 별도의 FW/Build ID 및
Fault 심볼 분석 화면, 콘솔 명령 입력,
...
TCP 포트만 연결되고 장치 명령 응답이 없는 경우에는 정상 연결과 구분해 주황색으로
표시합니다. 5초간 응답이 없으면 GUI가 자신이 시작한 OpenOCD를 재시작하여 ST-LINK/RTT
경로를 자동 재연결합니다. 반복 실패 시 재시도 간격은 최대 60초까지 늘어나며, 복구
과정에서 MCU reset/halt는 실행하지 않습니다. 상단의 `ST-LINK/RTT 재연결` 버튼으로도
같은 복구를 실행할 수 있습니다.
...
| HW | FW | Build ID |
|---|---|---|
| Rev3 | 3.2.0 | `4ba475e3bf5641aca4c98dd61e3d4310` |
